...
function preload() {
  soundFormats('mp3');
  mysound = loadSound('audio/illegalsubpassracing.mp3');
  mysound.setVolume(1);
  mysound.rate(0.7);
}
...
let cr = 20;
    ...
    let x =849;
    let y =318;
    // rect(cos(j)/2*i+x, sin(j)/2*i+y,i/20,i/20);

    j+=1.2;



    // background(color(mid*2,Math.abs(high-100*200),low,1));
    noFill();
    strokeWeight(i/1000);
    ...
    rotate(i);
    translate(j,j);
    quad(0,height,width,height,width,0,-1000,0);
    rotate(i);
